...
export default SceneLoader = {
    ...
    loadSceneRecursive(url, path = null) {
        ...
        return bundle.load().then(config => {
            ...
            if (!Array.isArray(config.import)) {
                SceneLoader.normalize(config, bundle);
                return config;
            }
    ...
    normalize(config, bundle) {
        SceneLoader.normalizeDataSources(config, bundle);
        SceneLoader.normalizeFonts(config, bundle);
        SceneLoader.normalizeTextures(config, bundle);
        return config;
    },
